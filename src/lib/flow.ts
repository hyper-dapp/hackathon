...
export async function makeFlow(code: string) {
  return await createFlow(code, {
    async onCallFn({ block, env, contractAddress, functionSig, paramTypes, args, returnType, value, mutability }: any) {
      ...
      const contract = new ethers.Contract(contractAddress, iface, env.provider)
      const result = await contract.connect(env.signer).functions[functionSig](...args, { value: value }).then(
        yes => console.log('yes', yes) || yes.map(convertEthersContractCallResult),
