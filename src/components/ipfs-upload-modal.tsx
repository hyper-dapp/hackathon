...
export const UploadModal = cc<Attrs>(function ($attrs) {
  ...
  const executePublish = async () => {
    ...
    try {
      const { name, description, external_url, image_url } = config;

      if (name && description) {
        const metaData = ipfs.createNFTMetaDataTemplate(
          pl,
          description,
          external_url,
          name,
    ...
    return (
      <div>
        ...
        <div class="mt-5 sm:mt-6">
          <button type="button" onclick={executePublish} class={btnClass()}>
            Upload
          </button>
        </div>
      </div>
